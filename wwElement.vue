...
<!-- Add this section to display streaming messages -->
<div class="streaming-messages" v-if="parsedMessages.length > 0">
<div v-for="(message, index) in parsedMessages" :key="index" class="message">
<div class="message-content">{{ message.content }}</div>
<div class="message-meta" v-if="content.showMetadata">
<span class="node-id">{{ message.nodeId }}</span>
...
// Add streaming-related variables and functions in the setup function
const streamingData = ref([]);
const isStreaming = ref(false);
const currentNodeId = ref(null);
const parsedMessages = ref([]);

// Add a method to process streaming data
...
currentNodeId.value = jsonData.metadata?.nodeId;
streamingData.value.push(jsonData);
} 
else if (jsonData.type === 'item') {
// Add the content to our streaming data array
streamingData.value.push(jsonData);

// For messages from AI Agent nodes, add to parsed messages
if (jsonData.metadata?.nodeName?.includes('AI Agent')) {
parsedMessages.value.push({
content: jsonData.content,
nodeId: jsonData.metadata?.nodeId,
timestamp: jsonData.metadata?.timestamp
});
}

// For JSON return nodes, try to parse the content
if (jsonData.metadata?.nodeName?.includes('Return JSON')) {
try {
const parsedJson = JSON.parse(jsonData.content);
if (parsedJson.reply) {
parsedMessages.value.push({
content: parsedJson.reply,
nodeId: jsonData.metadata?.nodeId,
timestamp: jsonData.metadata?.timestamp,
isJson: true
});
...
// Watch for changes to the items array when streaming is enabled
watch(() => props.content?.items, (newItems, oldItems) => {
if (!streamingEnabled.value || !Array.isArray(newItems) || !Array.isArray(oldItems)) return;

// If streaming is enabled, identify new items and add them to the buffer
if (newItems.length > oldItems.length) {
const newItemsCount = newItems.length - oldItems.length;
const newStreamItems = newItems.slice(newItems.length - newItemsCount);
streamBuffer.value = [...streamBuffer.value, ...newStreamItems];
}
}, { deep: true });

// Watch for changes to streaming configuration
watch(streamingEnabled, (isEnabled) => {
if (isEnabled) {
// Start the streaming interval
streamIntervalId = setInterval(processStreamData, streamingInterval.value);
} else if (streamIntervalId) {
// Clear the interval if streaming is disabled
clearInterval(streamIntervalId);
streamIntervalId = null;
}
});

// Watch for changes to the streaming interval
watch(streamingInterval, (newInterval) => {
if (streamingEnabled.value && streamIntervalId) {
// Update the interval timing
clearInterval(streamIntervalId);
streamIntervalId = setInterval(processStreamData, newInterval);
}
});

// Setup and cleanup for streaming
onMounted(() => {
...
const hasNewline = inputText.value.indexOf('\n') !== -1;

if (hasNewline) {
textareaInput.value.style.height = 'auto';
const newHeight = Math.min(textareaInput.value.scrollHeight, MAX_HEIGHT);
textareaInput.value.style.height = newHeight + 'px';
} else {
textareaInput.value.style.height = MIN_HEIGHT + 'px';
}
};
...
const userAttachments = selectedFiles.value.map(f => ({
name: f.name,
type: f.type,
thumb: f.preview,
url: null
...
addMessage(text, 'user', userAttachments);
...
// Watch for welcome message changes
watch(() => props.content?.welcomeMessage, (newVal) => {
if (newVal && messages.value.length === 0) {
addMessage(newVal, 'bot');
}
});

// Watch for content.streamingInput
